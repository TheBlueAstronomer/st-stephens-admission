...
describe('Rank 7 stronger view model regressions', () => {
  it('exports explicit applicant and interview detail query payload types', async () => {
    ...
    const applicantsQueryPath = path.resolve(__dirname, '../queries/applicants.ts');
    const interviewsQueryPath = path.resolve(__dirname, '../../interviews/queries/interviews.ts');
  ...
  it('applicant and interview detail UIs avoid authored runtime shape checks and page-level casts', async () => {
    const fs = await import('fs');
    const path = await import('path');
    const applicantTabsPath = path.resolve(__dirname, '../components/detail/tabs.tsx');
    const interviewPagePath = path.resolve(__dirname, '../../../app/(staff)/interviews/[id]/page.tsx');
    const applicantSharedPath = path.resolve(__dirname, '../components/detail/shared.tsx');
    const interviewSharedPath = path.resolve(__dirname, '../../interviews/components/detail/shared.ts');
