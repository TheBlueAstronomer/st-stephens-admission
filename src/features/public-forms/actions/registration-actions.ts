...
import { registrationFormSchema } from '@/features/public-forms/validations/registration-form';
import { findMatchingApplicant } from '@/features/applicants/services/duplicate-matching';
import { serializeAuditFields } from '@/lib/audit-log';
import type { AccommodationDuration, AccommodationType, Prisma } from '@/generated/prisma/client';
...
export async function submitRegistration(formData: FormData): Promise<SubmissionResult> {
  ...
  try {
    ...
    if (matchResult.confidence === 'LOW') {
      ...
      await prisma.pendingSubmission.create({
        data: {
          ...
          submittedData: raw as Prisma.InputJsonObject,
    ...
    const dbAccommodationType: AccommodationType | null = data.accommodationType === 'RESIDENTIAL' ? 'SINGLE' : null;
    const dbDuration: AccommodationDuration | null = data.accommodationDuration === 'FULL_YEAR' ? 'FULL_YEAR'
    ...
    await prisma.accommodationRequest.upsert({
      ...
      update: {
        ...
        accommodationType: dbAccommodationType,
        duration: dbDuration,
