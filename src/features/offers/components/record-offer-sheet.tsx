...
import { createOfferSchema, type CreateOfferInput } from '@/features/offers/validations/offer';
import { createOffer } from '@/features/offers/actions/offer-actions';
...
const OFFER_TYPE_OPTIONS = [
  { value: 'UNCONDITIONAL', label: 'Unconditional Offer', ariaLabel: 'Unconditional Offer', description: 'Full offer with no conditions attached.' },
  { value: 'CONDITIONAL', label: 'Conditional', ariaLabel: 'Conditional Offer', description: 'Offer subject to specified conditions.' },
  { value: 'DECLINED', label: 'Declined', ariaLabel: 'Declined', description: 'Application will not proceed.' },
  { value: 'WITHDRAWN', label: 'Withdrawn', ariaLabel: 'Withdrawn', description: 'Applicant has ended the process.' },
] as const;
...
export function RecordOfferSheet({
  ...
  const {
    ...
    resolver: zodResolver(createOfferSchema as never) as Resolver<CreateOfferInput>,
    defaultValues: {
      applicantId,
    ...
    <Sheet open={open} onOpenChange={handleClose}>
      <SheetContent className="w-[480px] sm:w-[540px] overflow-y-auto px-7 pb-10">
        <SheetHeader className="px-0 pt-8 pb-5 mb-1 border-b border-border">
          ...
          <SheetTitle className="text-xl font-semibold text-brand-ink leading-tight">Record Offer Decision</SheetTitle>
              ...
              {errors.conditions && (
                <p className="text-xs text-red-600">
                  {Array.isArray(errors.conditions)
                    ? (errors.conditions as Array<unknown> & { root?: { message?: string } }).root?.message ?? 'At least one condition is required.'
                    : (errors.conditions as { message?: string }).message}
                </p>
              )}
            </div>
          )}
