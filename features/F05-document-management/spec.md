...
- Document checklist section within the applicant detail screen (§5.6): all required document types from §6.8, showing required status, received status, waived status, received date, linked file, and notes.
...
- [ ] The document checklist displays all required document types from the PRD with correct required, received, and waived states for each applicant.
...
- [ ] Access to sensitive document types (Legal ID, DBS check, references, interview notes) is restricted to `ADMISSIONS_STAFF` and `SYSTEM_ADMINISTRATOR`.
